...
//
//----------------------------------------------------------------------//
USNMatchingTestRoomItem* USNMatchingJoinSessionMenu::ShowRoomItem(const FString& SessionName, int ConnectionNum, USNSessionSearchResult* SearchResult){
	...
	// ルーム情報を設定
	RoomItem->SetRoom(SessionName, ConnectionNum, SearchResult);
...
void USNMatchingJoinSessionMenu::OnJoinButtonClicked(USNButton* Button){
	...
	if(RoomItem != nullptr){
		// デリゲートを実行
		OnJoinButtonClickedDelegate.ExecuteIfBound(RoomItem->GetSearchResult());
	}
}
